...
async function main() {
  ...
  try {
    // Health
    const h = await fetchFn(`${BASE}/api/leads/health`);
    ...
    const ing = await fetchFn(`${BASE}/api/leads/ingest`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ rows }) });
    const ingJ = await ing.json();
    if (!ing.ok || !ingJ.ok) throw new Error('ingest failed');

    // Next batch
    const nb = await fetchFn(`${BASE}/api/leads/next-batch?count=2`);
    const nbJ = await nb.json();
    if (!Array.isArray(nbJ) || nbJ.length < 1) throw new Error('next-batch empty');

    // Mark first as sent
    const first = nbJ[0];
    const mk = await fetchFn(`${BASE}/api/leads/mark`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ url: first.url, status: 'sent' }) });
    const mkJ = await mk.json();
    if (!mk.ok || !mkJ.ok) throw new Error('mark failed');

    // Next batch again
    const nb2 = await fetchFn(`${BASE}/api/leads/next-batch?count=2`);
